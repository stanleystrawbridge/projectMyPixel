classdef projectMyPixel%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    methods%===============================================================        
        ...
        function obj = projectMyPixel(varargin)%---------------------------
            ...
            switch obj.method
                case 'linear'
                 ...
                 case 'radial'
                    obj = obj.radialMethod;
                case 'conical'
                    obj = obj.conicalMethod;
            end
        ...
        function obj = radialMethod(obj)%----------------------------------

            if numel(obj.manifold)==1
                ring = obj.manifold{1};
                focus = mean(obj.manifold{1});
            else
                ring = obj.manifold{1};
                focus = obj.manifold{2};
            end
           
            % Calculate distance from focus--------------------------------
            obj.projection_absolute = sum((obj.coordinates ...
                                - repmat(focus,obj.n_coordinates,1)).^2,2);


            % calculate frational distance between focus and ring----------
            obj.projection_fractional = zeros(obj.n_coordinates,1);

            % parallelize
            points = obj.coordinates;
            radii_lengths = zeros(obj.n_coordinates,1);
            distance_along_radii = radii_lengths;

            boundary =  [ring; ring(1,:)];          

            for i = 1:obj.n_coordinates                                              
                
                point = points(i,:);                            

                [radius_length, distance_along_radius] = ...
                         projectMyPixel.line2segment(focus,boundary,point);
                
                radii_lengths(i) = radius_length;
                distance_along_radii(i) = distance_along_radius;
                
            end

            obj.projection_fractional = distance_along_radii./radii_lengths;
            
        end%---------------------------------------------------------------
        
        
        function obj = conicalMethod(obj)%---------------------------------
            ...
            % Calculate distance from focus--------------------------------
            obj.projection_absolute = sum((obj.coordinates ...
                                - repmat(apex,obj.n_coordinates,1)).^2,2);
            ...
            for i = 1:obj.n_coordinates                                              
                ...
                [radius_length, distance_along_radius] = ...
                         projectMyPixel.line2segment(apex,arc,point);
